...
- [3. SQL](#3-sql)
  ...
  - [3-2. 데이터 조작 언어 (DML, Data Manipulation Language)](#3-2-데이터-조작-언어-dml-data-manipulation-language)
    ...
    - [(3) SELECT](#3-select)
      ...
      - [4. LIMIT](#4-limit)
    - [(4) SELECT문 실행 순서](#4-select문-실행-순서)
  ...
  - [3-4. 데이터 제어 언어 (DCL, Data Control Language)](#3-4-데이터-제어-언어-dcl-data-control-language)
    ...
    - [(2) REVOKE](#2-revoke)
  - [Q\&A](#qa)
    - [Q1. 데이터 정의어에 대해서 설명하고 각 명령어 종류를 설명해주세요.](#q1-데이터-정의어에-대해서-설명하고-각-명령어-종류를-설명해주세요)
    - [Q2. 데이터 조작어에 대해 설명하고 각 명령어 종류를 설명해주세요.](#q2-데이터-조작어에-대해-설명하고-각-명령어-종류를-설명해주세요)
    - [Q3. 트랜잭션 제어 언어에 대해 설명하고 각 명령어 종류를 설명해주세요.](#q3-트랜잭션-제어-언어에-대해-설명하고-각-명령어-종류를-설명해주세요)
...
## Q&A 
### Q1. 데이터 정의어에 대해서 설명하고 각 명령어 종류를 설명해주세요.
A1. 데이터 정의어는 데이터베이스 혹은 데이터베이스 객체를 생성, 수정, 삭제하는 언어입니다. 
그 종류로는 CREATE, ALTER, DROP, TRUNCATE가 있습니다.

CREATE 이후 데이터베이스, 테이블, 뷰, 인덱스를 붙여서 각 다양한 객체를 정의할 수 있습니다.
ALTER는 이미 생성된 테이블에 새로운 필드를 추가, 수정, 삭제할 수 있습니다. ALTER TALBLE 테이블 이름 ADD, CHANGE, DROP, ADD CONSTRAINT 중 하나를 사용한 후 필드 이름 필드타입 호은 제약 조건 이름을 작성하면 됩니다.
DROP은 테이블이나 데이터베이스를 삭제할 수 있습니다. TRUNCATE는 DROP과는 달리 테이블의 구조를 유지한 채로 테이블의 모든 레코드만 삭제합니다.


### Q2. 데이터 조작어에 대해 설명하고 각 명령어 종류를 설명해주세요.
A2.데이터 조작어는 테이블에 저장된 데이터를 조회, 추가, 수정, 삭제할 때 사용하는 언어입니다. 데이터 조작어에는 INSERT, UPDATE, DELETE, SELECT, GROUP BY, HAVING, ORDER BY, LIMIT이 있습니다.

먼저 INSERT는 테이블에 새로운 데이터를 추가할 때 사용하는 명령어입니다. 특정 필드에 값을 명시적으로 추가할 수 있습니다.

UPDATE와 DELETE는 테이블에 저장된 데이터를 수정하거나 삭제할 때 사용하는 명령어입니다. UPDATE는 특정 조건에 맞는 데이터를 수정할 때 사용하고, DELETE는 특정 조건에 맞는 데이터를 삭제할 때 사용합니다. 특히 외래키(Foreign Key) 제약 조건이 걸려 있을 경우 ON UPDATE, ON DELETE 옵션을 통해 연관된 테이블의 데이터가 함께 수정되거나 삭제되도록 설정할 수 있습니다.

SELECT는 테이블에서 데이터를 조회할 때 사용하는 명령어입니다. 특정 조건을 추가해 원하는 데이터를 필터링할 수 있습니다. SELECT에서는 패턴 검색과 집계 함수도 사용할 수 있습니다. 패턴 검색은 LIKE 키워드를 통해 특정 문자열 패턴과 일치하는 데이터를 검색할 수 있습니다.

GROUP BY는 SELECT 문에서 특정 기준에 따라 데이터를 그룹화할 때 사용합니다. HAVING은 GROUP BY로 그룹화한 결과에서 특정 조건을 추가해 필터링할 때 사용합니다. ORDER BY는 조회된 데이터를 특정 기준에 따라 정렬할 때 사용하며, 오름차순은 ASC, 내림차순은 DESC를 사용합니다. LIMIT은 조회된 결과에서 출력할 개수를 제한할 때 사용합니다. 예를 들어, 상위 5개의 데이터만 출력하고 싶을 때 사용합니다.

데이터 조작어는 트랜잭션의 영향을 받기 때문에 COMMIT 이전에는 ROLLBACK을 통해 작업을 취소할 수 있으며 문법 순서와는 달리 `FROM → WHERE → GROUP BY → HAVING → SELECT → ORDER BY → LIMIT `순서대로 실행됩니다.

### Q3. 트랜잭션 제어 언어에 대해 설명하고 각 명령어 종류를 설명해주세요.
A3. 트랜잭션 제어 언어는 트랜잭션을 제어하고 처리 결과를 저장하거나 취소할 때 사용하는 언어입니다. 트랜잭션 제어 언어에는 COMMIT, ROLLBACK, SAVEPOINT, SET TRANSACTION이 있습니다.

COMMIT은 트랜잭션에서 수행한 작업을 확정하고 저장할 때 사용하는 명령어입니다. 트랜잭션에서 COMMIT을 실행하면 해당 작업이 데이터베이스에 반영됩니다.

ROLLBACK은 트랜잭션에서 수행한 작업을 취소하고 이전 상태로 되돌릴 때 사용하는 명령어입니다. 트랜잭션에서 문제가 발생했을 때 ROLLBACK을 실행하면 작업이 수행되기 전 상태로 복구됩니다.

SAVEPOINT는 트랜잭션 내에서 특정 지점을 저장할 때 사용하는 명령어입니다. 트랜잭션 진행 중 SAVEPOINT를 설정하면 해당 시점까지 작업 상태를 저장하고 필요할 경우 해당 시점까지 롤백할 수 있습니다.