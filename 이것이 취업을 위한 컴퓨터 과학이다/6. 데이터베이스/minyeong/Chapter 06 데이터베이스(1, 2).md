...
- [06-2. RDBMS의 기본](#06-2-rdbms의-기본)
  ...
  - [무결성 제약 조건](#무결성-제약-조건)
- [Q\&A](#qa)
  - [1. 차수와 카디날리티가 무엇인지 설명해주세요.](#1-차수와-카디날리티가-무엇인지-설명해주세요)
  - [2. 트랜잭션이 지켜야 하는 성질에 대해 설명해주세요.](#2-트랜잭션이-지켜야-하는-성질에-대해-설명해주세요)
  - [3. 기본 키와 외래 키의 차이점을 설명해주세요.](#3-기본-키와-외래-키의-차이점을-설명해주세요)
...
# Q&A
## 1. 차수와 카디날리티가 무엇인지 설명해주세요.
차수는 테이블에서 속성(컬럼)의 개수를 의미하며, 카디널리티는 한 속성(컬럼)에 대해 서로 다른 고유한 값의 개수를 의미합니다

## 2. 트랜잭션이 지켜야 하는 성질에 대해 설명해주세요.
트랜잭션은 데이터베이스에서 하나의 논리적인 작업 단위이며, 다음 네 가지 성질을 지녀야 합니다.  
첫 째, 원자성은 트랜잭션 내 모든 작업이 완전히 수행되거나 전혀 수행되지 않아야 함을 의미합니다.  
둘 째, 일관성은 트랜잭션이 실행되기 전과 후의 데이터가 항상 일관된 상태를 유지해야 하는 것 입니다.  
셋 째, 격리성은 동시에 여러 트랜잭션이 실행될 때 서로 간섭하지 않는 것 입니다.  
넷 째, 지속성은 트랜잭션이 성공적으로 완료되면 그 변경 내용이 영구적으로 반영되어야 하는 것을 의미합니다.

## 3. 기본 키와 외래 키의 차이점을 설명해주세요.
기본 키는 테이블 내에서 각 레코드를 고유하게 식별하는 키이며, 중복을 허용하지 않고 NULL 값을 가질 수 없습니다.  
반면, 외래 키는 다른 테이블의 기본 키를 참조하는 키로, 참조하는 기본 키 값과 같거나 NULL 값을 가질 수 있으며, 중복된 값이 존재할 수도 있습니다.