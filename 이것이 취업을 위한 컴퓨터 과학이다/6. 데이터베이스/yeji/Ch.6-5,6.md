...
#### 2. IE 표기법 (Information Engineering Notation) = 새 발/까마귀 발 표기법(Crow Feet Notation)
   - 오늘 날 RDBMS에서 사용
...
| 분할 방식 | 설명 |
|-----------|------|
| 범위 분할 (Range Partitioning) | 특정 값의 범위로 나눔 (예: ID 1~1000) |
| 목록 분할 (List Partitioning) | 명시적 목록(리스트) 기준 분할 (예: 지역명, 카테고리 등) |
| 해시 분할 (Hash Partitioning) | 해시 함수로 균등 분산 |
| 키 분할 (Key-based Sharding) | 샤딩 키를 기준으로 샤드에 데이터 분배 |
