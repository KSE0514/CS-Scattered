# Chapter 06-4. 효율적 쿼리
- [Chapter 06-4. 효율적 쿼리](#chapter-06-4-효율적-쿼리)
  - [서브 쿼리와 조인](#서브-쿼리와-조인)
...
![INNER JOIN(내부 조인), OUTER JOIN(외부 조인), FULL OUTER JOIN(완전 외부 조인)](이미지/06join종류.png)
...
- **인덱스(Index)** : **검색 속도 향상**을 목적으로 만드는 하나 이상의 데이터 필드에 대한 자료구조
    - 수많은 레코드를 조회하는 작업이 빈번한 RDBMS의 **성능을 향상**시키는 가장 대중적인 방법
    - 특정 필드에 대한 인덱스를 생성하면 원하는 레코드에 더 빠르게 접근 가능
    - 한 테이블에 2개의 인덱스가 있을 수 있음
    - 테이블(필드)에 종속된 개념이기 때문에 테이블이 삭제되면 관련된 인덱스는 모두 삭제됨
    - 인덱스의 대표적인 자료 구조 : 해시테이블, B트리
        <details>
        ...
        **B+Tree** : B-Tree에서 리프 노드 간 연결성이 추가된 자료구조
        ...
        ![각 노드에 키로써 인덱스 값 포함됨, 인덱스 값을 탐색하면 실제 데이터(레코드)가 저장된 위치 알 수 있음](이미지/06b-tree.png)
