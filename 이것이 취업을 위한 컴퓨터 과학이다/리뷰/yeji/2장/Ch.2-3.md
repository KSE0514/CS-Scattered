...
### ➕ 슈퍼스칼라(superscalar)
- CPU 내부에 여러 명령어 파이프라인 포함하는 구조<br>
  ![슈퍼스칼라](image-6.png)
- <참고> 명령어 파이프라이닝과 슈퍼스칼라 비교
  | 구분       | 명령어 파이프라이닝          | 슈퍼스칼라                    |
  | -------- | ------------------- | ------------------------ |
  | 처리 단위    | 한 명령어를 여러 단계로 나눔    | 여러 명령어를 동시에 실행           |
  | 병렬 수준    | 명령어의 **단계 간 병렬화**   | 명령어 **자체의 병렬화**          |
  | 비유       | 하나의 햄버거 조립 라인       | 햄버거 조립 라인을 여러 개 둠        |
  | 성능 향상 방식 | 순차 명령어를 겹쳐서 빠르게 처리함 | 실행할 수 있는 명령어를 찾아 동시에 실행함 |
