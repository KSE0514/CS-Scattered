...
- [Q\&A](#qa)
  - [1. 깊이 우선 탐색과 너비 우선 탐색의 차이를 설명해주세요.](#1-깊이-우선-탐색과-너비-우선-탐색의-차이를-설명해주세요)
  - [2. BFS와 다익스트라 알고리즘의 차이점은 무엇인가요?](#2-bfs와-다익스트라-알고리즘의-차이점은-무엇인가요)
  - [3. 트리와 그래프는 어떤 관계인가요?](#3-트리와-그래프는-어떤-관계인가요)
...
---
# Q&A
## 1. 깊이 우선 탐색과 너비 우선 탐색의 차이를 설명해주세요.
깊이 우선 탐색과 너비 우선 탐색은 그래프를 탐색하는 방법입니다.  
깊이 우선 탐색은 그래프에서 더 이상 방문 가능한 정점이 없을 때까지 최대한 깊은 자식 노드까지 탐색하기를 반복하는 탐색 방법입니다.  
너비 우선 탐색은 인접한 모든 정점들을 방문하고, 방문한 정점들과 연결된 모든 정점을 방문하기를 반복하는 탐색 방법입니다.

## 2. BFS와 다익스트라 알고리즘의 차이점은 무엇인가요?
BFS와 다익스트라 알고리즘의 차이는 가중치의 유무입니다.  
BFS는 간선의 가중치가 없거나 모두 동일한 경우(=비용이 같을 경우)에 사용할 수 있고,
다익스트라는 모든 간선의 가중치가 0 이상일 때 사용하는 알고리즘입니다.  
다만, 간선 가중치가 0 또는 1처럼 제한적일 경우에는,
다익스트라 대신 BFS가 더 효율적일 수 있습니다.  
실제 구현 측면에서는 BFS가 큐를 사용하고, 다익스트라는 최소 힙(우선순위 큐)을 사용한다는 점도 차이입니다.

## 3. 트리와 그래프는 어떤 관계인가요?
트리는 그래프의 한 종류로, 사이클이 없고 모든 노드가 하나로 연결된 무방향 그래프입니다.  
일반적인 그래프는 사이클이 있거나 여러 연결 요소로 나뉠 수 있지만, 트리는 항상 연결되어 있고, 노드가 N개라면 간선은 N-1개라는 특징을 갖습니다.