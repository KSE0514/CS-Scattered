...
- 특징
    ...
    - 스레드마다 각각의 프로그램 카운터 값과 스택 가짐
        - 스레드마다 다음에 실행할 주소와 연산 과정의 임시 저장 값 가질 수 있음

**멀티프로세스와 멀티스레드의 차이**

|  | 멀티프로세스 | 멀티스레드 |
| --- | --- | --- |
| 자원의 공유 여부 | 자원 공유 X | 자원 공유 O
(코드, 데이터, 힙, 파일) |
| 장점 | 한 프로세스에 문제 생겨도 다른 프로세스에 지장 없거나 적음 | 스레드 간 쉽게 협력 및 통신 가능 |
| 단점 | 같은 프로그램 실행하기 위해 메모리에 동일한 내용 중복 존재 | 한 스레드에 생긴 문제가 프로세스 전체의 문제가 될 수 있음 |
<details> 
