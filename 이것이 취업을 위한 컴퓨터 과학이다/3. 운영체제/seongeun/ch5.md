...
## 파일과 디렉터리
### 파일
- 파일(file): 파일의 이름, 파일을 실행하기 위한 정보, 파일과 관련한 부가 정보(속성(attribute) or 메타데이터(metadata))로 구성되어 있음
![alt text](image-125.png)
![alt text](image-126.png)
- 파일을 다루는 모든 작업이 운영체제에 의해 이뤄짐
  - 응용 프로그램 -> 임의로 파일을 할당받아 조작하고 저장할 수 없음. 파일을 다루는 시스템 콜을 이용해야 함
...
- '아이노드 번호'로 설명된 부분이 '파일이 저장된 위치를 유추할 수 있는 정보'에 해당
...
> **파티셔닝**
> - 보조기억장치의 영역을 구획하는 작업
> - 파티션(partition): 파티셔닝되어 나누어진 하나 하나의 영ㅇ역
...
- ![alt text](image-149.png)
  - '/dev/sda' 장치에 있는 EXT4파일 시스템을 (-t ext4) 읽기 전용으로 (-o ro) /mnt/test 디렉터리에 마운트(mount)하라는 의미의 명령어
...
Q1. 연결 할당과 색인 할당의 디렉터리 엔트리에는 각각 어떤 정보가 명시되는지 말해보시오.

A1. 연결 할당의 경우 디렉터리 엔트리에 파일 이름, 첫 번째 블록 주소, 파일을 이루는 블록 단위의 길이가 명시됩니다. 반면 색인 할당의 경우 파일을 이루는 모든 블록의 주소를 색인 블록이라는 특별한 블록에 모아 관리하기 때문에 디렉터리 엔트리에는 파일 이름과 색인 블록의 주소가 명시됩니다. 

Q2. 포매팅이란 무엇인가요?

A2. 포매팅이란 파일 시스템을 설정하여 어떤 방식으로 저장하고 관리할 것인지를 결정하고, 새로운 데이터를 쓸 준비를 하는 작업을 말합니다.

Q3. 마운트란 무엇인가요?

A3. 마운트란 어떤 저장장치의 파일 시스템에서 다른 저장장치의 파일 시스템으로 접근할 수 있도록 파일 시스템을 편입시키는 작업을 말합니다.