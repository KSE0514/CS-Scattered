# Ch.5-5 응용 계층 - HTTP의 기초
- [Ch.5-5 응용 계층 - HTTP의 기초](#ch5-5-응용-계층---http의-기초)
- [1. DNS와 URI/URL](#1-dns와-uriurl)
  - [1.1 도메인 네임과 DNS](#11-도메인-네임과-dns)
  - [1.2 자원과 URI/URL](#12-자원과-uriurl)
- [2. HTTP의 특징과 메시지 구조](#2-http의-특징과-메시지-구조)
  - [2.1 HTTP의 특징](#21-http의-특징)
    - [HTTP 버전별 특징](#http-버전별-특징)
- [3. HTTP 메시지 구조](#3-http-메시지-구조)
  - [3.1 시작 라인(Start Line)](#31-시작-라인start-line)
  - [3.2 HTTP 메서드와 상태 코드](#32-http-메서드와-상태-코드)
    - [HTTP 중요 메서드](#http-중요-메서드)
    - [HTTP 상태 코드](#http-상태-코드)
- [4. HTTP 주요 헤더](#4-http-주요-헤더)
  - [4.1 요청 메시지에 주로 사용되는 HTTP 헤더](#41-요청-메시지에-주로-사용되는-http-헤더)
  - [4.2 응답 메시지에 주로 사용되는 HTTP 헤더](#42-응답-메시지에-주로-사용되는-http-헤더)
  - [4.3 요청과 응답 메시지 모두에서 활용되는 HTTP 헤더](#43-요청과-응답-메시지-모두에서-활용되는-http-헤더)
    - [표현 헤더(Representation Header)](#표현-헤더representation-header)
- [Question](#question)
  - [Q1. HTTP는 스테이트리스(stateless) 프로토콜이라고 하는데, 이를 설명하고 장점과 단점을 말해보세요.](#q1-http는-스테이트리스stateless-프로토콜이라고-하는데-이를-설명하고-장점과-단점을-말해보세요)
  - [Q2. HTTP 상태 코드 301과 302의 차이를 설명하고, 308과 307이 추가된 이유는?](#q2-http-상태-코드-301과-302의-차이를-설명하고-308과-307이-추가된-이유는)
  - [Q3.  HTTP 요청에서 GET과 POST의 차이를 설명하고, 각각의 사용 사례를 들어보세요.](#q3--http-요청에서-get과-post의-차이를-설명하고-각각의-사용-사례를-들어보세요)
