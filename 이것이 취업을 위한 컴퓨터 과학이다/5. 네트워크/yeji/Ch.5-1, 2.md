...
- 전송 방식
  ...
  - **브로드캐스트(Broadcast)**: 네트워크 내 모든 호스트에 데이터 전송
    - 브로드캐스트 도매인(Broadcast Domain): 브로드캐스트가 전송되는 범위
    - 호스트가 같은 브로드케스트 도메인에 속해 있는 경우, 같은 LAN에 속해 있다고 간주
...
- 참고
  ...
  - SSID(Service Set Identifier): 서비스 셋 식별자 ex) dhkdlvkdl dlfma
...
- **NIC(Network Interface Controller)**: 컴퓨터와 네트워크를 연결하는 하드웨어
  ...
  - 예전에는 호가장 카드 형태였지만 최근 다양한 형태로 변화
...
브로드캐스트 도매인(Broadcast Domain)이라는 개념이 있으며, 이는 브로드캐스트가 전송되는 범위를 의미합니다.
호스트가 같은 브로드케스트 도메인에 속해 있는 경우, 같은 LAN에 속해 있다고 간주합니다.

## Q2. 캡슐화 역캡슐화에 대해 설명해주세요.
...
- 여러 대의 호스트를 연결합니다.
- 리피터 허브라고 부르기도 합니다.
- 전달 받은 신호를 모든 포트로 내보냅니다 (신호를 전달받으면 어떠한 조작/판단 없이 모든 포트에 단순히 신호를 내보냄).
- 반이중 모드로 통신합니다: 한 번에 한 방향으로만 통신 가능(동시 송수신 불가).
- 충돌 도메인이 넓습니다. 두 장치가 동시에 데이터 전송 시 충돌이 발생할 수 있습니다.

데이터 링크 계층의 스위치
- 데이터 링크 계층에서 동작하며, MAC 주소 기반으로 패킷을 전달합니다.
- 허브와 달리 전달받은 신호를 목적지 호스트가 연결된 포트로만 내보냅니다.
- 전이중 모드를 지원합니다 (동시 송수신 가능).
- 허브보다 충돌 도메인이 좁습니다.
- MAC 주소 학습 기능으로 전달받은 신호를 원하는 포트에만 내보낼 수 있습니다.
- MAC 주소 테이블을 통해 학습한 MAC 주소와 포트 정보를 저장합니다.
- VLAN(Virtual LAN) 기능을 통해 하나의 물리적 네트워크를 여러 논리적 네트워크로 분할할 수 있습니다.