- [Ch.5-6 응용 계층 - HTTP의 응용](#ch5-6-응용-계층---http의-응용)
...
- 특정 서버로부터 쿠키 전달받으면 다음부터 해당 서버 요청 보낼 때 전달받은 쿠키 자동으로 전송
> Cookie: 이름=값; 이름=값;
...
### 대표적인 콘텐츠 협상 헤더
- `Accept`: 선호하는 미디어 타입 나타내는 헤더 (예: `text/html, application/json`)
...
- 이렇게 서버-클라이언트 간 암호화에 사용할 키 획득하고 서로 인증하면 마지막으로 TLS 핸드셰이크 마지막인 Finished 메시지를 주고받고 이후부터 TLS 핸드셰이크 통해 얻어낸 키를 기반으로 암호화된 데이터 주고 받음<br>
